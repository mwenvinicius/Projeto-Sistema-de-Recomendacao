...
prvd = Dados["Providers"]
linksPrvd = Dados["LinksProviders"]
application = Dados["Applications"]
interationApp = Dados["Interations"]
...
# O nome do arquivo de interação.
def dicoveryName(nameApp):
    nameInte = ""
    for i in range(len(nameApp)):
        if(nameApp[i] == '-'):
            nameInte = '../Interations/APP'+ nameApp[i-1] + '-iterations-1.json'
    return nameInte
